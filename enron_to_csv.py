...
def load_and_convert_enron():
    # Code taken from https://github.com/MWiechmann/enron_spam_data/blob/master/build_data_file.py
    ...
    if not os.path.exists("dataset/enron"):
        os.mkdir("dataset/enron")
    ...
    for entry in enron_list:
        ...
        r = requests.get(url)
        path = "dataset/enron/" + entry + ".tar.gz"

        with open(path, 'wb') as f:
            f.write(r.content)
        print('...done! Archive saved to: ' + path)

        # Unpack current archive this will unpack to /dataset/enron/enron1, etc
        print("Unpacking contents of " + entry + " archive...")
        shutil.unpack_archive(path, "dataset/enron/")
        print("...done! Archive unpacked to: dataset/enron/" + entry)
    ...
    for directory in enron_list:
        ...
        ham_folder = "dataset/enron/" + directory + "/ham"
        spam_folder = "dataset/enron/" + directory + "/spam"
        i = 0
    ...
    mails.to_csv('dataset/enron/enron_spam_data.csv', index_label="Message ID")
